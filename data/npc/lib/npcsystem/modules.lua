...
if Modules == nil then
	...
	function ShopModule:callbackOnBuy(cid, itemid, subType, amount, ignoreCap, inBackpacks)
		...
		if inBackpacks then
			totalCost = isItemStackable(itemid) == TRUE and totalCost + 2 or totalCost + (math.max(1, math.floor(amount / getContainerCapById(backpack))) * 2)
		end
		...
		if a < amount then
			...
			if a > 0 then
				doPlayerRemoveMoney(cid, ((a * shopItem.buy) + (b * 2)))
				return true
			end
